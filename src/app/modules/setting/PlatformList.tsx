...
import { AccountModel ,PlatformModel} from 'app/modules/setting/models/Account'

import PlatformItemLimit from './components/PlatformItemLimit'
...
type Props = {
  className: string,
  accounts: PlatformModel[]
}

const PlatformList: React.FC<Props> = ({ className, accounts }) => {
        ...
        <div className="page-header__content">
          <div className="align-items-center row" style={{margin:10}}>
            <div className="col-lg-12 col-sm-12 c-order__header">
              <span  className='fw-bolder fs-3 mb-1'>Platforms Priority</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          ...
          <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4'>
            ...
            <tbody>
              {
                  accounts&&accounts?.map((item: PlatformModel,index:number) => {
                      return <PlatformItemLimit key={item.platform} item={item} index={index} />
                    })
              }
