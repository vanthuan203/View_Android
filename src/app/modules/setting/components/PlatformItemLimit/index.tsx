...
const PlatformItemLimit : React.FC<Props> = ({ item ,index}) => {
    const dispatch = useDispatch()
    const [loading, setLoading] = useState(false)
    const clickHandler =()=>{
        dispatch(actions.showCurrentPlatformLimit(item))
    }
    return (
        ...
        <tr style={{margin:100,backgroundColor:item.state==0?"rgba(252,226,207,0.62)":"#ffffff"}}>
            ...
            <td >
                    <span style={{fontSize:11}} >
                                <text style={{fontWeight:"bold",color:item.activity==1?"#088114":"#a22727"}} >
                                        {item.activity==1?"ON":"OFF"}
                                </text>
                    </span>
            </td>
            <td >
...
export default PlatformItemLimit