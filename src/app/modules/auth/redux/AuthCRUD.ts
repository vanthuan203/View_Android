...
export async function login(email: string, password: string) {
  const res = await postWithoutTokenFunciton("user/login", {
    username: email,
    password: password,
  })
  return res
...
export async function register(username: string, password: string) {
  const res = await postWithoutTokenFunciton("user/register", {
    username: username,
    password: password,
  })
  return res
...
export async function getUserByToken() {
  ...
  const res:any = await getFunciton("user/verify_token")
  return res
}
