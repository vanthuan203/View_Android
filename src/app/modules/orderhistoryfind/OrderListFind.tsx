            ...
            <table className='table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3'>
              ...
              {orders &&
                  orders.map((order: OrderModel, index: number) => {
                    ...
                    }else if((key.indexOf(order.order_key)>=0 || order.note.indexOf(key)>=0  || key.indexOf("vn")>=0&&order.service_id>=600 || key.indexOf("us")>=0&&order.service_id<600 || key.indexOf(order.order_id.toString()) >=0 || order.service_id.toString().indexOf(key.indexOf('?')>=0?key.replace('?',''):'done')>=0)
                    ...
                    else if(((key.indexOf(order.order_key)>=0 || order.note.indexOf(key)>=0  || key.indexOf("vn")>=0&&order.service_id>=600 || key.indexOf("us")>=0&&order.service_id<600 || key.indexOf(order.order_id.toString()) >=0 || order.service_id.toString().indexOf(key.indexOf('?')>=0?key.replace('?',''):'done')>=0) && order.username.indexOf(keyuser)>=0 )
